...
const axiosInstance = axios.create({
  baseURL: `${backendBaseUrl}/webui`,  // Add /webui to the base URL
  headers: {
...
> => {
  try {
    const response = await axiosInstance.get('/webui/health')  // Use /webui/health instead of /health
    return response.data
  } catch (e) {
    return {
      status: 'error',
...
export const getAuthStatus = async (): Promise<AuthStatusResponse> => {
  try {
    ...
    // Default to guest mode if response is invalid
    return {
      auth_configured: false,
      auth_mode: 'disabled',
      access_token: 'guest-token',
    ...
    // If the request fails, use guest mode
    console.error('Failed to get auth status:', errorMessage(error));
    return {
      auth_configured: false,
      auth_mode: 'disabled',
      access_token: 'guest-token',
      message: '访客模式已启用 (服务器连接失败)'
    };
  }
}
